...
std::vector<cv::Mat> erode(const cv::Mat& src_erode, const cv::Mat& src_dilate) {
    ...
    // TODO: 在这里实现你的代码
    cv::Mat gray_erode, binary_erode, dst_erode;
    cv::Mat gray_dilate, binary_dilate, dst_dilate;

    cvtColor(src_erode, gray_erode, cv::COLOR_BGR2GRAY);
    cvtColor(src_dilate, gray_dilate, cv::COLOR_BGR2GRAY);
    ...
    cv::dilate(binary_dilate, dst_dilate, kernel, cv::Point(-1, -1), 1);
